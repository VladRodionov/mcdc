...
static int zstd_load_dict(zstd_ctx_t *ctx){
	...
	while (off < (size_t) file_size) {
		size_t n = fread((char*) buf + off, 1,
				(size_t) file_size - off, f);
	...
	atomic_store_explicit(&ctx->cdict,
			(uintptr_t) ZSTD_createCDict(buf, file_size,
					ctx->cfg.level), memory_order_release);
	atomic_store_explicit(&ctx->ddict,
			(uintptr_t) ZSTD_createDDict(buf, file_size),
...
static int zstd_save_dict(const void *dict,
                          size_t      dict_size,
						  zstd_ctx_t *ctx,
						  bool        overwrite)
{
		...
		if (ZSTD_isError(dict_sz)) { /* hard failure      */
			...
			if (nc && nd) {
			    ...
			    if (ctx->cfg.dict_path) {   /* reuse same path or another config field */
			        int rc = zstd_save_dict(dict, dict_sz,
			                                       ctx, /* or other path */
			                                       /* overwrite = */ true);
			        if (rc)
			            log_rate_limited(10*1000000ULL,
